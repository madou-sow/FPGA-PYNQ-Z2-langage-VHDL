...
continue;
fs << (pointsIter -> _x - cx) * scale + W/2 << " " << (pointsIter -> _y - cy) * scale + H/2 << "
c\n";
}
fs << "\n0 setgray " << (centersIter -> _x - cx) * scale + W/2 << " " << (centersIter -> _y - cy) *
scale + H/2 << " s\n";
}
fs << "\n%%%%EOF";
delete[] colors;
...
if (pointsIter -> _group != k)
continue;
doc << svg::Circle(svg::Point((pointsIter -> _x - cx) * scale + W/2, (pointsIter -> _y - cy) *
scale + H/2), 1, svg::Fill(svg::Color::White), svg::Stroke(1, c));
}
doc << svg::Rectangle(svg::Point((centerIter -> _x - cx) * scale + W/2, (centerIter -> _y - cy) *
scale + H/2), 5, 5, svg::Fill(svg::Color::White), svg::Stroke(2, c));
}
delete[] colors;
doc.save();
}
